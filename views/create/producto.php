...
if (empty($nombre) || empty($descripcion) || empty($precio) || empty($stock) || empty($imagen)) {
    ...
    if ($result > 0) {
        ...
        $sql = "INSERT INTO `producto` (`id_categoria`, `id_proveedor`, `nom_producto`, `imagen`, `descripcion`, `precio`, `stock`) VALUES ('$id_categoria', '$id_proveedor', '$nombre', 'ruta/imagen', '$descripcion', '$precio', '$stock')";
        $query = mysqli_query($con, $sql);
